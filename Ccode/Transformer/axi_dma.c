...
#define TEST_SIZE 173056
u8 Source_TX[TEST_SIZE] = {0};
u8 Source_RX[TEST_SIZE] = {0};
u8 test_data = 0;
void AXI_DMA_IP_TEST(void)
{
	...
	Xil_DCacheFlushRange((UINTPTR)(&Source_TX), TEST_SIZE);
	Xil_DCacheFlushRange((UINTPTR)(&Source_RX), TEST_SIZE);

	// Start DDR to IP transfer
	AXI_DMA_Transfer((UINTPTR)(&Source_TX), TEST_SIZE, XAXIDMA_DMA_TO_DEVICE);

	// Start IP to DDR transfer
	AXI_DMA_Transfer((UINTPTR)(&Source_RX), TEST_SIZE, XAXIDMA_DEVICE_TO_DMA);
